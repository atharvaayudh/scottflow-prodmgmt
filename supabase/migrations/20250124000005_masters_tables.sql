-- Masters Tables Migration
-- This creates all the master data tables for the production management system
...
CREATE TABLE IF NOT EXISTS fabric_master (
  ...
  composition TEXT, -- 100% Cotton, 60% Cotton 40% Polyester
...
CREATE TABLE IF NOT EXISTS item_master (
  ...
  item_type VARCHAR(100), -- Raw Material, Finished Good, Semi-Finished
  category_id UUID REFERENCES product_categories(id),
  description TEXT,
  unit_of_measure VARCHAR(20), -- pieces, meters, kg, etc.
  specifications JSONB, -- Flexible specifications
  is_active BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS branding_master (
  ...
  typography VARCHAR(100), -- Font family
...
CREATE TABLE IF NOT EXISTS size_master (
  ...
  size_type VARCHAR(50), -- Clothing, Footwear, Accessories
...
CREATE TABLE IF NOT EXISTS customization_master (
  ...
  customization_type VARCHAR(100), -- Embroidery, Printing, Stitching, etc.
  description TEXT,
...
CREATE TABLE IF NOT EXISTS customer_master (
  ...
  customer_type VARCHAR(50), -- Individual, Corporate, Retailer, etc.
  contact_person VARCHAR(255),
  email VARCHAR(255),
  phone VARCHAR(20),
  address TEXT,
...
CREATE TABLE IF NOT EXISTS vendor_master (
  ...
  vendor_type VARCHAR(50), -- Supplier, Manufacturer, Service Provider
  contact_person VARCHAR(255),
  email VARCHAR(255),
  phone VARCHAR(20),
  address TEXT,
  ...
  gstin VARCHAR(15),
  pan VARCHAR(10),
  bank_details JSONB, -- Account number, IFSC, etc.
  payment_terms VARCHAR(100),
  is_active BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
